...
async function main() {
  const inboxId = 1; // Ajustar si tienes otro ID de bandeja (inbox)
  const botToken = "hzeNLSTYNCqPuVpTfm6kgkXM";
  
  console.log(`Insertando Bot en Supabase para Inbox ${inboxId}...`);
  
  const { data, error } = await supabase
    .from('clientes_bot')
    .upsert([{
      inbox_id: inboxId,
      chatwoot_token: botToken,
      chatwoot_url: process.env.CHATWOOT_BASE_URL || 'https://soporte.mbtech.work',
      ai_model: process.env.MODELO_POR_DEFECTO || 'google/gemma-4-31b-it',
      system_prompt: `Eres BotWaba, un asistente de ventas experto en neuromarketing y persuasión. Tu objetivo principal es guiar a los prospectos a través del embudo de ventas utilizando el protocolo AIDA (Atención, Interés, Deseo, Acción).
...
- Tu tono debe ser cordial, profesional, persuasivo y humano.`
  ...
  if (error) {
    ...
    console.log("✅ ¡Registro insertado/actualizado con éxito!");
    console.log(data);
  }
}
